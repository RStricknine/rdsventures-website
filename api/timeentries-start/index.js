...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const openCheck = await pool.request()
      .input("EmployeeProfileId", sql.UniqueIdentifier, lookup.EmployeeProfileId)
      .query(`
        SELECT TOP 1
          te.TimeEntryId,
          te.WorkDate,
    ...
    const existingOpen = openCheck.recordset && openCheck.recordset[0];
    if (existingOpen) {
      context.res = json(409, {
        ok: false,
        error: "You already have an open time entry.",
        openEntry: existingOpen
      });
      return;
    }
    ...
    insertRequest.input("HoursWorked", sql.Decimal(10, 2), 0);
    insertRequest.input("IsManualHours", sql.Bit, 0);
    ...
    context.log.error("timeentries/start error", err);
